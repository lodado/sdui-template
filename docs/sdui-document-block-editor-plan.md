...
- [x] 드래그 중 프레임당 React 리렌더 0회(A안 채택 — overlay DOM 직접 갱신)
- [x] 포커스/선택 변경 시 영향 row만 리렌더 (renderCount probe로 검증)
- [x] patch 커밋 시 변경 경로 밖 블록 리렌더 0회 (path-copy + memo, Enter split probe로 검증)
- [x] 기존 유닛/E2E 전부 그린, public API 불변

### 구현 기록 (2026-07-04)

- 21.1~21.3을 한 커밋으로 병합 (focus/selection store 없이는 memo가 무의미해 분리 불가).
- **발견한 숨은 전량-리렌더 원인**: `useSensor(PointerSensor, {…})`의 인라인 옵션 객체가
  매 렌더 새 레퍼런스 → sensors identity 변경 → DndContext 내부 컨텍스트 재생성 →
  모든 row의 memo 무력화. 옵션을 모듈 상수로 호이스팅해 해결.
- Escape 처리 순서 버그 수정: PM keymap이 만든 selection을 컨테이너 keydown이
  같은 이벤트에서 즉시 해제하던 문제 — store는 동기 조회라 노출됨.
  `event.defaultPrevented` 가드로 해결.
- probe 기법: `jest.mock`으로 BlockChrome을 감싸 block id별 렌더 횟수 계측.

---

## Phase 22. Selection 포맷팅 popover — PM mark 토글 + Radix Popover (Outline 이식)

텍스트를 드래그 선택하면 뜨는 포맷팅 툴바. mark 토글(bold/italic/strikethrough/
underline/code/highlight/link)과 highlight 색상 팔레트를 제공한다.
디자인·동작·값은 전부 Outline(`app/editor/components/SelectionToolbar.tsx`,
`FloatingToolbar.tsx`, `menus/formatting.tsx`, `marks/*`)에서 이식 — 창작 금지.
포지셔닝 엔진만 Outline의 자체 계산 대신 **Radix Popover**를 쓴다(요구사항).

### 현행 갭 (분석 결과)

| 항목         | 현재                                                                                  | Outline                                                   | 필요 작업          |
| ------------ | ------------------------------------------------------------------------------------- | --------------------------------------------------------- | ------------------ |
| mark 종류    | bold/italic/code/link 4종 (`sdui-document/src/blocks/schema/inline.ts`)               | + strikethrough, underline, highlight(color attr)         | 도메인 스키마 확장 |
| PM 스키마    | 4 mark (`focused-block/pm/schema.ts`)                                                 | 7 mark                                                    | mark 3종 추가      |
| keymap       | Mod-b/i/e                                                                             | + Mod-d(strike), Mod-u(underline), Mod-Shift-h(highlight) | keymap 추가        |
| selection UI | 없음                                                                                  | SelectionToolbar + FloatingToolbar                        | 신규               |
| radix        | sdui-document-react에 없음 (sdui-template-component에 @radix-ui/react-popover@1.1.15) | —                                                         | 의존성 추가        |

**Outline에 없는 것 (구현 안 함)**: 텍스트 색상(font color) mark — Outline은
highlight(배경색)만 지원. "색상 변경" = highlight 색상 팔레트로 해석.
폰트 패밀리/크기 변경도 Outline에 없음 → 범위 제외. 필요해지면 별도 phase.

### 이식할 Outline 값 (정확한 출처)

**highlight 팔레트** (`shared/utils/color.ts:75-82`, opacity 0.4 `marks/Highlight.ts:12`):

```text
#FDEA9B Coral | #FED46A Apricot | #FA551E Sunset
#B4DC19 Smoothie | #C8AFF0 Bubblegum | #3CBEFC Neon
```

**mark toDOM/parseDOM** (`shared/editor/marks/*`):

| mark          | toDOM                                                 | parseDOM                                      | keymap      | input rule |
| ------------- | ----------------------------------------------------- | --------------------------------------------- | ----------- | ---------- |
| strikethrough | `<del>`                                               | s/del/strike, `text-decoration: line-through` | Mod-d       | `~text~`   |
| underline     | `<u>`                                                 | u, `text-decoration-line: underline`          | Mod-u       | `__text__` |
| highlight     | `<mark data-color style="background: rgba(hex, .4)">` | mark[data-color]                              | Mod-Shift-h | `==text==` |

**툴바 디자인** (`FloatingToolbar.tsx`, `ToolbarMenu.tsx`, `ToolbarButton.tsx`,
`theme.ts`):

- 높이 36px, padding 6px, 버튼 24×24px, 버튼 간격 6px, radius 4px
- separator: 1px × 36px, textTertiary 25% opacity, margin -6px 2px
- 배경: light `#FFFFFF` / dark `#181c25`
- shadow light: `0 0 0 1px rgb(0 0 0/2%), 0 4px 8px rgb(0 0 0/8%), 0 2px 4px rgb(0 0 0/0%), 0 30px 40px rgb(0 0 0/8%)`
- shadow dark: `0 0 0 1px rgb(34 40 52), 0 8px 16px rgba(0,0,0,.3), 0 2px 4px rgba(0,0,0,.08)`
- 버튼: opacity .8 → hover 1, active 시 배경 `accent`(#0366d6) + 글자 `accentText`(#fff), transition 100ms
- 등장 애니메이션: 150ms cubic-bezier + 150ms delay, scale(.95)→1 + translateY(-6px), opacity 0→1
- 버튼 순서 (formatting.tsx): bold, italic, strikethrough, code | highlight(서브메뉴) | link

**동작 규칙** (SelectionToolbar.tsx / FloatingToolbar.tsx):

- 표시: selection 비어있지 않을 때. 숨김: readOnly, 드래그 선택 진행 중
  (mousedown~mouseup `isSelectingText` 플래그), 블록 드래그 중
- 위치: selection rect 상단 중앙 (`view.coordsAtPos(from/to)`)
- 툴바 클릭이 에디터 focus/selection을 깨지 않도록 popover에서 `onOpenAutoFocus`
  방지 + 버튼 `onMouseDown={e => e.preventDefault()}`
- highlight 버튼: 활성 색이 있으면 그 색 원형 아이콘, 클릭 시 팔레트 서브메뉴
  (6 preset + None)

### 22-A. 도메인 mark 확장 (sdui-document)

- `blocks/schema/inline.ts`: `SduiInlineMark` union에
  `{ type: 'strikethrough' } | { type: 'underline' } | { type: 'highlight', attrs: { color: string } }` 추가. zod 스키마 동기화 (color는 `#RRGGBB` regex 검증 — 경계 검증 원칙).
- 오프셋 산정 로직(`content/inlineContent.ts`)은 mark 무관 — 변경 없음.
- 테스트: zod 검증(유효/무효 hex 경계), 직렬화 round-trip.

### 22-B. PM 스키마 + 정적 렌더러 + CSS

- `pm/schema.ts`: 3 mark 추가 (위 표의 toDOM/parseDOM 그대로).
  highlight `excludes` 없음(Outline과 동일), code는 기존 유지.
- `pm/serialization.ts`: `toPmMark`/`toSduiMark`에 highlight attrs(color) 매핑.
- `pm/editorState.ts`: keymap Mod-d/Mod-u/Mod-Shift-h + input rules(`~~`, `__`, `==`).
- `inline/InlineContentView.tsx`: `wrapWithMark`에 del/u/mark 추가 —
  mark는 `<mark data-color style={{backgroundColor: rgba(color, .4)}}>`.
  PM 뷰와 정적 뷰 시각 동일(파리티) 유지.
- `styles/editor.css`: `--sdui-doc-highlight-*` 팔레트 토큰 6개,
  `[data-sdui-document-editor] mark` 규칙.
- 테스트: 직렬화 round-trip(마크 중첩·경계), InlineContentView 렌더 태그 검증,
  keymap 토글.

### 22-C. SelectionToolbar — Radix Popover

새 폴더 `src/selection-toolbar/`:

```text
selection-toolbar/
├── SelectionToolbar.tsx     # Radix Popover.Root + Anchor + Content
├── ToolbarButton.tsx        # 24px 버튼, active/disabled 상태
├── HighlightPalette.tsx     # 6색 + None 서브뷰
├── useEditorSelection.ts    # PM selection → React 상태 (플러그인 경유)
└── __tests__/
```

- **selection 관찰**: `FocusedBlockEditor`에 PM 플러그인 1개 추가 —
  `view()` 훅에서 매 트랜잭션 후 `{from, to, empty, activeMarks}` 를
  `onSelectionChange` 콜백으로 방출. React 쪽은 focused row 내부 state
  (Phase 21 원칙 유지: focused row만 리렌더, 다른 row 0회 — renderCount probe로 검증).
- **앵커**: Radix `Popover.Anchor`를 zero-size absolute div로 두고
  `view.coordsAtPos(from)`/`coordsAtPos(to)` rect로 이동 (컨테이너 상대 좌표,
  drop indicator overlay와 같은 기법). Radix가 collision/flip 처리.
- **open 조건**: `!empty && !readOnly && !isSelectingText && !isDragging`.
  `isSelectingText`: 에디터 mousedown~mouseup 플래그(Outline 방식).
- **mark 토글**: 버튼 클릭 → `toggleMark(schema.marks.x)(view.state, view.dispatch)`
  → PM 트랜잭션 → commit 채널은 기존 blur/위임 흐름 그대로 (즉시 커밋 안 함 —
  마크는 blur 시점에 block.update로 저장. 기존 계약 불변).
- **active 상태**: storedMarks ?? `rangeHasMark` — Outline `isMarkActive` 로직 이식.
- **focus 유지**: `Popover.Content`의 `onOpenAutoFocus`/`onCloseAutoFocus` preventDefault,
  버튼 `onMouseDown` preventDefault → PM selection 유지.
- **link**: 툴바 두 번째 상태(LinkEditor) — href input + 적용/제거.
  Outline처럼 selection에 link mark 있으면 자동으로 link 편집 모드.
  scheme whitelist는 기존 `safeHref` 재사용.
- 의존성: `@radix-ui/react-popover` (sdui-template-component와 동일 1.1.x).
- CSS: 위 "툴바 디자인" 값 그대로 `--sdui-doc-menu-*` 토큰으로.
- 테스트(P0): 선택 → 툴바 표시 / collapsed → 숨김 / bold 토글 후 static 뷰 반영 /
  active 표시 / Escape 닫기 / readOnly 숨김 / renderCount(타 row 0회) /
  highlight 색 적용 attrs round-trip. jsdom에서 `coordsAtPos` 미지원 시
  anchor 위치만 mock, 로직은 실 PM state로.

### 22-D. E2E + Storybook

- Storybook: AllBlocks에 mark 조합 fixture 추가(highlight 6색 문단),
  Editable 스토리로 수동 검증.
- E2E(chromium): 더블클릭 단어 선택 → 툴바 등장 → Bold 클릭 → `<strong>` 확인 →
  highlight 색 적용 → `mark[data-color]` 확인.

### 커밋 순서

1. 22-A — sdui-document mark 스키마 (단독 그린)
2. 22-B — PM/정적 렌더러/CSS 파리티
3. 22-C — SelectionToolbar (radix 의존성 포함)
4. 22-D — E2E + Storybook + 이 문서에 구현 기록
