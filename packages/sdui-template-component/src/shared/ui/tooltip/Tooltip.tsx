...
/**
 * SimpleTooltip - Legacy API wrapper for backward compatibility
 *
 * @description
 * A simplified tooltip component that wraps the compound pattern internally.
 * Use this for simple tooltip use cases or backward compatibility.
 *
 * @example
 * ```tsx
 * <SimpleTooltip content="Add to library" side="top" showArrow>
 *   <button>+</button>
 * </SimpleTooltip>
 * ```
 */
...
const SimpleTooltip = ({
  ...
  return (
    <TooltipPrimitive.Root
      ...
      delayDuration={delayDuration}
    >
      <TooltipPrimitive.Trigger asChild data-node-id={nodeId}>
        {children}
      </TooltipPrimitive.Trigger>
      <TooltipPrimitive.Portal>
        <TooltipPrimitive.Content
        ...
        >
          {content}
          {showArrow && <TooltipPrimitive.Arrow className={cn(tooltipArrowVariants())} width={11} height={5} />}
        </TooltipPrimitive.Content>
      </TooltipPrimitive.Portal>
    </TooltipPrimitive.Root>
  )
