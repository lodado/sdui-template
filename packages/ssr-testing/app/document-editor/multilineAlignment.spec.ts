...
test.describe('Shift-Enter 멀티라인 정렬', () => {
  ...
  test('멀티라인 블록에서 드래그 핸들과 + 버튼은 첫 줄 옆에 고정된다', async ({ page }) => {
    ...
    expect(
      Math.abs((await centerY(page.locator('[data-block-id="p1"] [data-drag-handle]'))) - firstLineCenter),
    ).toBeLessThanOrEqual(TOLERANCE)
    expect(
  ...
  test('한 줄 블록의 핸들 위치는 기존과 동일하게 행 중앙이다 (회귀 방지)', async ({ page }) => {
    ...
    expect(
      Math.abs((await centerY(page.locator('[data-block-id="p2"] [data-drag-handle]'))) - rowCenter),
    ).toBeLessThanOrEqual(TOLERANCE)
  })
  ...
  test('멀티라인 h1에서도 핸들이 첫 줄 기준으로 정렬된다', async ({ page }) => {
    ...
    expect(
      Math.abs((await centerY(page.locator('[data-block-id="p2"] [data-drag-handle]'))) - firstLineCenter),
    ).toBeLessThanOrEqual(TOLERANCE)
  })
})
