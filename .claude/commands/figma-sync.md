...
## 4.4 Refresh MCP Knowledge

```bash
pnpm --filter @lodado/sdui-mcp build
```

- Required whenever `sdui-template-component` source or stories changed — MCP serves compressed knowledge generated from them; skipping this ships stale guides to consumers.
...
- Add changeset if the package's published behavior changed: `pnpm changeset` (patch for style-only, minor for new variants/props)
