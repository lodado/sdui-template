...
import { BadgeRow, Callout, CodeSnippet, DemoFrame, DocHero, DocPage, DocSection, Prose } from './components'

const meta: Meta = {
  title: 'Document/Architecture/6. sdui-template-component (Library)',
  parameters: {
    layout: 'fullscreen',
    docs: {
      description: {
        component:
          'SDUI 렌더러에 꽂아 쓰는 Radix 기반 컴포넌트 라이브러리 @lodado/sdui-template-component — 통합 레지스트리, 컴파운드 컴포넌트, 폼 검증, Canvas3D 렌더 전략 주입.',
      },
    },
  },
}
...
const Render = ({ document }: { document: SduiLayoutDocument }) => (
...
const LibraryPage = () => {
  return (
    <DocPage accent="components">
      <DocHero
        kicker="Package · @lodado/sdui-template-component"
        title="렌더러에 꽂는 Radix 기반 컴포넌트 세트"
        lead="sdui-template 이 '어떻게 그릴지'라면, 이 패키지는 '무엇을 그릴지'입니다. Button·Dialog·Dropdown·Form 등 40여 개 타입을 하나의 맵으로 제공해 SduiLayoutRenderer 에 그대로 주입합니다."
        pills={['sduiComponents', 'createSduiComponents', 'compound', 'registerSchemas', 'Radix UI', 'Canvas3D']}
      />

      <DocSection index="6.1" label="Registry" title="통합 레지스트리 · 한 맵으로 주입">
        <Prose>
          <p>
            <code>sduiComponents</code> 는 타입 문자열을 <code>ComponentFactory</code> 로 잇는 하나의 맵입니다. 렌더러의{' '}
            <code>components</code> 로 넘기면 끝. 주입점(예: Canvas3D 렌더 전략)이 필요하면{' '}
            <code>createSduiComponents(options)</code> 로 맵을 만들어 넘깁니다.
          </p>
        </Prose>
        <CodeSnippet file="app/sduiComponents.tsx" code={REGISTRY_CODE} />
        <DemoFrame title="Basic components" hint="모두 같은 sduiComponents 맵으로 렌더">
          <Render document={basicsDoc} />
        </DemoFrame>
      </DocSection>

      <DocSection index="6.2" label="Compound" title="컴파운드 컴포넌트 · 타입으로 분해">
        <Prose>
          <p>
            Dialog·Dropdown·Popover 같은 복합 위젯은 <strong>여러 SDUI 타입의 트리</strong>로 표현됩니다. 부모 타입이
            열림/선택 상태를 소유하고 자식 타입은 context로 소비하므로, JSON 트리 구조가 그대로 컴파운드 구조가 됩니다.
          </p>
        </Prose>
        <CodeSnippet file="shared/ui/dropdown/*" code={COMPOUND_CODE} />
        <DemoFrame title="Dropdown compound" hint="트리거를 눌러 열기 — 상태는 Dropdown 노드가 소유">
          <Render document={dropdownDoc} />
        </DemoFrame>
        <BadgeRow
          items={[
      ...
      <DocSection index="6.3" label="Forms" title="폼 검증 · zod 스키마 등록">
        <Prose>
          <p>
        ...
        <CodeSnippet file="features/form/types.ts" code={FORM_CODE} />
        <BadgeRow items={['registerSchemas', 'registerSchema', 'getSchema', 'react-hook-form', 'zod resolver']} />
      </DocSection>

      <DocSection index="6.4" label="Injection" title="Canvas3D · 렌더 전략 주입">
        <Prose>
          <p>
            무거운 3D 의존성을 코어에 넣지 않기 위해 <code>Canvas3D</code> 는 <strong>기본 렌더 전략이 없습니다</strong>
            . 소비자가 <code>createSduiComponents({'{ canvas3DRenderStrategy }'})</code> 로 전략을 주입해야 실제로
            그려집니다 — 의존성 역전으로 번들과 관심사를 분리하는 패턴.
          </p>
        </Prose>
        <CodeSnippet file="shared/ui/canvas-3d/*" code={CANVAS_CODE} />
        <Callout icon="◆">
