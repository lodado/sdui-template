    ...
    docs: {
      description: {
        component:
          ...
          'Checked items are implemented today; unchecked items are scheduled in ' +
          '`docs/sdui-document-block-editor-plan.md` Phase 24 (Outline parity + Notion-style block selection). ' +
...
const divider = () => createDocumentBlock({ id: `shortcut-${(sequence += 1)}`, type: 'document.divider' })
...
const shortcutGuide: SduiDocumentContent = {
  ...
  root: createDocumentBlock({
    ...
    children: [
      ...
      callout('info', t('체크된 항목 = 구현 완료 (이 문서에서 바로 시험 가능). 빈 체크 = Phase 24 예정.')),
      divider(),
      ...
      item(false, kbd('Mod-Shift-C'), t(' — 인라인 코드 대체 바인딩 (24-B)')),
      ...
      paragraph(
        ...
        t(' 위임 → '),
        t('block.set', { type: 'code' }),
        t(' patch), 마크 패턴은 PM 내부에서 즉시 적용된다.'),
      ),
      item(true, kbd('# '), t(' / '), kbd('## '), t(' / '), kbd('### '), t(' — heading 1~3')),
      item(true, kbd('[] '), t(' — checklist')),
      ...
      item(false, kbd('**텍스트**'), t(' — '), t('굵게', { type: 'bold' }), t(' (24-B)')),
      item(false, kbd('*텍스트*'), t(' / '), kbd('_텍스트_'), t(' — '), t('기울임', { type: 'italic' }), t(' (24-B)')),
      item(false, kbd('`텍스트`'), t(' — '), t('인라인 코드', { type: 'code' }), t(' (24-B)')),
      item(false, kbd('---'), t(' — divider (블록은 있음, rule만 누락)')),
      item(
        false,
        kbd('- '),
        t(' bullet / '),
      ...
      item(false, kbd('Ctrl-Shift-0/1/2/3/7'), t(' — paragraph / h1~h3 / checklist로 turn-into (24-C)')),
      item(false, kbd('Mod-]'), t(' / '), kbd('Mod-['), t(' — Tab/Shift-Tab alias (24-D)')),
      item(
        false,
        kbd('Mod-Alt-ArrowUp'),
        t(' / '),
        kbd('Mod-Alt-ArrowDown'),
        t(' — 블록을 형제 사이에서 위/아래 이동 (24-D)'),
      ),
      item(false, kbd('Shift-Enter'), t(' — hard break (노드는 있음, 바인딩 누락 — 24-D)')),
      item(false, kbd('Mod-Enter'), t(' — 컨텍스트 액션: checklist 토글, link 열기(http/https만) (24-E)')),
      divider(),
      ...
      item(false, kbd('ArrowUp'), t(' / '), kbd('ArrowDown'), t(' — 평탄화 순서로 selection 이동 (24-F)')),
      item(false, kbd('Enter'), t(' — 선택 블록 인라인 편집 진입, 커서는 끝 offset (24-F)')),
      item(
        false,
        kbd('Mod-D'),
        t(' — 블록 복제(subtree + 새 id). 인라인 편집의 취소선과 모드로 분리되어 충돌 없음 (24-F)'),
      ),
      item(false, kbd('Mod-A'), t(' — 전체 블록 선택. Notion식 텍스트→블록→전체 단계 확장 (24-F)')),
      divider(),
